...
function VI(X, a1, b1, a2, b2, phi, max_iter)
    N = size(X, 1)
    phi_h = deepcopy(phi)
    ...
    for i in 1 : max_iter
        ...
        delta2 = cumsum(phi_h)
        a1_h = a1 + sum(X.*delta1)
        b1_h = b1 + sum(delta1)
        a2_h = a2 + sum(X.*delta2)
        b2_h = b2 + sum(delta2)
        ...
        for n in 1 : N
            ln_p1[n] = X[n] * (digamma(a1_h) - log(b1_h)) - a1_h/b1_h
            ln_p2[n] = X[n] * (digamma(a2_h) - log(b2_h)) - a2_h/b2_h
        end
...
if test
    ...
    s = rand(Categorical(phi))
    X = Array{Float64, 1}(N)
    for n in 1 : s
        X[n] = rand(Poisson(lambda1))
    end
    for n in s+1 : N
        X[n] = rand(Poisson(lambda2))
    end
else
...
if test
    figure("data samples")
    clf()
    bar(1:N, X, color="#348ABD")
    plot([1, s], [lambda1, lambda1], "g--")
    plot([s+1, N], [lambda2, lambda2], "g--")
    plot([s+0.5, s+0.5],[0, maximum(X)],"r--")
    xlim([0, N])
else
    figure("data samples")
    clf()
...
# inference
max_iter = 100
phi_est, lambda1_est, lambda2_est = VI(X, a1, b1, a2, b2, phi, max_iter)

# calc expectation
s_expt = sum(phi_est .* range(1,N))
...
clf()
subplot(2,1,1)
bar(1:N, X, color="#348ABD", label="受信数")
...
plot([s_expt, s_expt], [0, maximum(phi_est)], "r--", label="期待値")
xlim([0, N])
ylabel("事後確率")
xlabel("経過日数")
legend()
